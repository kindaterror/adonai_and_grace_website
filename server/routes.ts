...
app.post("/api/students/:id/reject", authenticate, authorize(["admin"]), async (req, res) => {
  ...
  // =========================
  // Bulk Upsert Pages (Hybrid save support)
  // =========================
  app.put("/api/books/:bookId/pages/bulk", authenticate, authorize(["admin", "teacher"]), async (req, res) => {
    const bookId = parseNumericId(req.params.bookId);
    if (!bookId) return res.status(400).json({ success: false, message: "Invalid book id" });
    const { pages: incomingPages } = req.body || {};
    if (!Array.isArray(incomingPages)) {
      return res.status(400).json({ success: false, message: "'pages' array required" });
    }

    if (incomingPages.length > 200) {
      return res.status(413).json({ success: false, message: "Too many pages in one request (max 200)" });
    }
    ...
    try {
      ...
      // Fetch current pages once
      const existingPages = await db.query.pages.findMany({
        where: eq(schema.pages.bookId, bookId),
        with: { questions: true },
      });
      const existingById = new Map<number, any>();
      const pageNumberInUse = new Set<number>();
      for (const p of existingPages) {
        existingById.set(p.id, p);
        pageNumberInUse.add(p.pageNumber);
      }

      const createdIdMap: Record<string, number> = {}; // tempId -> new id
      const duplicateConflicts: Array<{ pageNumber: number; reason: string }> = [];

      await db.transaction(async (tx) => {
        for (const raw of incomingPages) {
          if (!raw) continue;
          const {
            id,
            _tempId,
            ...
            // basic validation
          if (pageNumber == null || String(content).trim() === "") continue; // skip invalid 

          if (id && existingById.has(Number(id))) {
            // UPDATE
            const updateData: any = {
              title: String(title || ""),
              content: String(content).trim(),
            ...
            if (typeof shuffleQuestions === "boolean") updateData.shuffleQuestions = shuffleQuestions;
            await tx.update(schema.pages).set(updateData).where(eq(schema.pages.id, Number(id)));
            // Replace questions
            await tx.delete(schema.questions).where(eq(schema.questions.pageId, Number(id)));
            if (Array.isArray(questions)) {
              for (const q of questions) {
            ...
            if (pageNumberInUse.has(num)) {
              duplicateConflicts.push({ pageNumber: num, reason: "Duplicate pageNumber" });
              continue;
            }
            const [newPage] = await tx
              ...
              .returning();
            pageNumberInUse.add(num);
            if (_tempId && newPage?.id) createdIdMap[String(_tempId)] = newPage.id;
            if (Array.isArray(questions) && newPage?.id) {
              for (const q of questions) {
                if (!q?.questionText || !q.questionText.trim()) continue;
                await tx.insert(schema.questions).values({
      ...
      // Return fresh canonical list
      const finalPages = await db.query.pages.findMany({
        where: eq(schema.pages.bookId, bookId),
        orderBy: asc(schema.pages.pageNumber),
        with: { questions: true },
      });
      ...
      console.error("PUT /api/books/:bookId/pages/bulk error:", err);
      return res.status(500).json({ success: false, message: "Internal server error" });
    }
  });
